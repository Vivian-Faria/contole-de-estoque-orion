...
export default function App() {
  ...
  const [falhaStorage, setFalhaStorage] = useState(false);

  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get(K_CATALOGO, true);
        if (r?.value) {
          const c = JSON.parse(r.value);
          setCatalogo({
            categorias: c.categorias ?? [],
            itens: c.itens ?? [],
            hubs: c.hubs?.length ? c.hubs : HUBS_PADRAO,
            pessoas: c.pessoas?.length ? c.pessoas : PESSOAS_PADRAO,
          });
        }
      } catch (e) {
        if (!window.storage) setFalhaStorage(true);
      }
      try {
        const r = await window.storage.get(K_CONTAGENS, true);
        if (r?.value) setContagens(JSON.parse(r.value));
      } catch (e) {
        if (!window.storage) setFalhaStorage(true);
      }
      setCarregando(false);
  ...
  const persistir = useCallback(async (chave, valor) => {
    try {
      await window.storage.set(chave, JSON.stringify(valor), true);
    } catch (e) {
      setFalhaStorage(true);
    }
  }, []);

  const salvarCatalogo = (novo) => {
    setCatalogo(novo);
    persistir(K_CATALOGO, novo);
  };

  const registrar = (contagem) => {
    const novas = [...contagens, contagem];
    setContagens(novas);
    persistir(K_CONTAGENS, novas);
  };

  const excluirContagem = (id) => {
  ...
  const carregarExemplo = () => {
    ...
    setCatalogo(cat);
    setContagens(cts);
    persistir(K_CATALOGO, cat);
    persistir(K_CONTAGENS, cts);
    setAba("consumo");
  };
  ...
  const limparTudo = () => {
    ...
    setCatalogo(vazio);
    setContagens([]);
    persistir(K_CATALOGO, vazio);
    persistir(K_CONTAGENS, []);
  };
      ...
      <main className="conteudo">
        {falhaStorage && (
          <div style={{ marginBottom: 16 }}>
            <Aviso
              tipo="erro"
              texto="Não foi possível salvar os dados neste ambiente. O que você registrar agora vale só até fechar a página."
            />
          </div>
        )}
